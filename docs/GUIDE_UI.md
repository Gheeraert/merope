...
**Raccourcis clavier** : Ctrl+B (gras), Ctrl+I (italique), Ctrl+Maj+S (barré), Ctrl+Maj+= (exposant), Ctrl+Espace ou Alt+Espace (espace insécable), Ctrl+Maj+V (coller en texte brut, sans mise en forme), Alt+J (bascule le paragraphe courant entre aligné à gauche et justifié, comme sous WordPress).
...
**Notes de bas de page** : un panneau dédié sous la zone de texte liste toutes les notes du document ([1], [2]...), avec un champ modifiable pour chacune — le texte de la note se corrige directement là, sans dialogue séparé. Cliquer sur un appel de note `[N]` dans le texte donne le focus à la note correspondante dans le panneau. **Supprimer** retire une note, y compris une note vide (les appels de note existants dans le texte ne sont pas retirés automatiquement).

**Numérotation des notes** : à l'enregistrement, les notes sont automatiquement renumérotées dans l'ordre de leur premier appel dans le texte (1, 2, 3...), quel que soit l'ordre dans lequel elles ont été créées. Cela permet d'ajouter, de supprimer ou de déplacer des notes librement en cours de rédaction sans se soucier de trous ou d'un ordre incohérent : les appels `[N]` dans le texte et le panneau de notes sont mis à jour ensemble. Une note dont l'appel a été supprimé à la main dans le texte (plutôt que via **Supprimer** dans le panneau) n'est pas effacée, mais reléguée après toutes les notes effectivement appelées.

**Raccourci `((...))`** : comme sur les carnets Hypothèses, un texte entouré de doubles parenthèses (`((comme ceci))`) est reconnu comme une note différée — y compris lorsque la note est directement collée à la ponctuation qui la suit (ex. `((comme ceci)).`). Dans l'éditeur, à la frappe comme au collage, cette syntaxe reste volontairement du texte ordinaire et entièrement modifiable : on peut donc mettre une partie de son contenu en gras, en italique ou en lien avant l'enregistrement. La conversion en note Pandoc n'intervient qu'au moment de la normalisation utilisée par l'aperçu réel et la génération du site. Les parenthèses simples (ex. `(a)`), les blocs de code (` ```...``` `) et le code en ligne (`` `comme ceci` ``) ne sont pas convertis.
...
- `src/bloggen/markdown/note_shortcuts.py` — conservation du raccourci `((note))` comme texte éditable dans l'éditeur, puis conversion en note Pandoc uniquement lors de la normalisation d'aperçu/génération.
